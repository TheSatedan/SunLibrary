...
class faq extends SunLibraryModule {
    ...
    public function faq() {
        ?>
        <b>Frequently Asked Questions</b> <a href="web-settings.php?id=Faq&&moduleID=AddFaq"><button>Add New</button></a>
        <br><br><br>
        <table width="100%" cellpadding="10" cellspacing="0" border="0">
            <tbody>
                ...
                while ($checkRow = $stmt->fetch()) {
                    ?>
                    <tr class="rowRef" style="" onclick="location.href = 'web-settings.php?id=Faq&&moduleID=EditFaq&&faqID=<?= $faqID; ?>'">
                        <td width="40%">
                            <?= $faqQuestion; ?>
                        </td>
                    </tr>
    ...
    public function editFaq() {
        ...
        if ($stmt = $this->objDB->prepare("SELECT faqID, faqQuestion, faqAnswer, faqOrder FROM faq WHERE faqID=? ")) {
            ...
            ?>			
                        ...
                        <tr>
                            <td>
                                <textarea rows=15 name="area2" style="width: 940px; background-color: white;"><?= $faqAnswer; ?></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input type="Submit" name="Submit" value="Update">
                                <a href="?id=Faq&&moduleID=DeleteFaq&&faqID='<?= $faqID; ?>"><button>Delete</button></a>
                            </td>
                        </tr>
            <?php
        }
